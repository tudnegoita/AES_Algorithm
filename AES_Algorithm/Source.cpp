#include <iostream>
#include <vector>
#include <cstring>

const unsigned char sbox[256] = {
    
};

unsigned char aes_gf_mul(unsigned char a, unsigned char b) {
    unsigned char p = 0;
    unsigned char hi_bit_set;
    for (int counter = 0; counter < 8; counter++) {
        if ((b & 1) == 1)
            p ^= a;
        hi_bit_set = (a & 0x80);
        a <<= 1;
        if (hi_bit_set == 0x80)
            a ^= 0x1B;
        b >>= 1;
    }
    return p;
}

void aes_key_expansion(const unsigned char* key, unsigned char* expanded_key) {
    const unsigned char rcon[11] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 0x6C };
    int i, j, k;
    unsigned char temp[4];

    memcpy(expanded_key, key, 16);

    for (i = 4; i < 44; i++) {
        memcpy(temp, expanded_key + (i - 1) * 4, 4);
        if (i % 4 == 0) {
            unsigned char temp_byte = temp[0];
            for (j = 0; j < 3; j++)
                temp[j] = temp[j + 1];
            temp[3] = temp_byte;

            for (j = 0; j < 4; j++)
                temp[j] = sbox[temp[j]];

            temp[0] ^= rcon[i / 4 - 1];
        }
        for (j = 0; j < 4; j++)
            expanded_key[i * 4 + j] = expanded_key[(i - 4) * 4 + j] ^ temp[j];
    }
}

void aes_sub_bytes(unsigned char* state) {
    for (int i = 0; i < 16; i++)
        state[i] = sbox[state[i]];
}

void aes_shift_rows(unsigned char* state) {
    unsigned char temp;

    temp = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = temp;

    temp = state[2];
    state[2] = state[10];
    state[10] = temp;
    temp = state[6];
    state[6] = state[14];
    state[14] = temp;

    temp = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7] = state[3];
    state[3] = temp;
}

void aes_mix_columns(unsigned char* state) {
    unsigned char tmp[4];
    for (int i = 0; i < 4; i++) {
        tmp[0] = aes_gf_mul(state[i * 4], 2) ^ aes_gf_mul(state[i * 4 + 1], 3) ^ state[i * 4 + 2] ^ state[i * 4 + 3];
        tmp[1] = state[i * 4] ^ aes_gf_mul(state[i * 4 + 1], 2) ^ aes_gf_mul(state[i * 4 + 2], 3) ^ state[i * 4 + 3];
        tmp[2] = state[i * 4] ^ state[i * 4 + 1] ^ aes_gf_mul(state[i * 4 + 2], 2) ^ aes_gf_mul(state[i * 4 + 3], 3);
        tmp[3] = aes_gf_mul(state[i * 4], 3) ^ state[i * 4 + 1] ^ state[i * 4 + 2] ^ aes_gf_mul(state[i * 4 + 3], 2);
        memcpy(state + i * 4, tmp, 4);
    }
}

void aes_add_round_key(unsigned char* state, const unsigned char* round_key) {
    for (int i = 0; i < 16; i++)
        state[i] ^= round_key[i];
}

void aes_encrypt(const unsigned char* input, const unsigned char* key, unsigned char* output) {
    unsigned char state[16];
    memcpy(state, input, 16);
    unsigned char expanded_key[176];
    aes_key_expansion(key, expanded_key);


    aes_add_round_key(state, key);


    for (int i = 1; i <= 9; i++) {
        aes_sub_bytes(state);
        aes_shift_rows(state);
        aes_mix_columns(state);
        aes_add_round_key(state, expanded_key + i * 16);
    }

   
    aes_sub_bytes(state);
    aes_shift_rows(state);
    aes_add_round_key(state, expanded_key + 160);

    
    memcpy(output, state, 16);
}

int main() {
    
    unsigned char plaintext[] = {
        0x32, 0x88, 0x31, 0xe0,
        0x43, 0x5a, 0x31, 0x37,
        0xf6, 0x30, 0x98, 0x07,
        0xa8, 0x8d, 0xa2, 0x34
    };

    unsigned char key[] = {
        0x2b, 0x28, 0xab, 0x09,
        0x7e, 0xae, 0xf7, 0xcf,
        0x15, 0xd2, 0x15, 0x4f,
        0x16, 0xa6, 0x88, 0x3c
    };

    unsigned char ciphertext[16];
    aes_encrypt(plaintext, key, ciphertext);
    std::cout << "Ciphertext: ";
    for (int i = 0; i < 16; i++)
        std::cout << std::hex << (int)ciphertext[i] << " ";
    std::cout << std::endl;

    return 0;
}
